...
public class TeleportUtilityOld
{
	...
	private static void teleportEntityTo(Entity entity, Vec3d pos, float yaw, float pitch)
	{
		...
		entity.setLocationAndAngles(pos.x, pos.y, pos.z, yaw, pitch);
	...
	private static Vec3d getClampedDestinationPosition(Vec3d posIn, World worldDst)
	{
		...
		double x = MathHelper.clamp(posIn.x, border.minX() + 2, border.maxX() - 2);
		double y = MathHelper.clamp(posIn.y, -4096, 4096);
		double z = MathHelper.clamp(posIn.z, border.minZ() + 2, border.maxZ() - 2);
	...
	{
		WorldServer worldDst = server.getWorld(dimension);
		if (worldDst == null)
		{
			return entity;
		}
